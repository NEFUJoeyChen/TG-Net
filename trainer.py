...
from model.networks_v2 import Generator, LocalDis, GlobalDis
...
class Trainer(nn.Module):
    ...
    def forward(self, x, bboxes, mask, ground_truth, compute_loss_g=False):
        self.train()
        l1_loss = nn.L1Loss()
        losses = {}
        ...
        if compute_loss_g:
            ...
            losses['ae'] = l1_loss(x1 * (1. - mask), ground_truth * (1. - mask)) * \
                self.config['coarse_l1_alpha'] + \
    ...
    def resume(self, checkpoint_dir, epoch=0, test=False):
        # Load generators
        last_model_name = get_model_list(checkpoint_dir, "gen", iteration=epoch)
        self.netG.load_state_dict(torch.load(last_model_name))
        epoch = int(last_model_name[-11:-3])

        if not test:
            # Load discriminators
            last_model_name = get_model_list(checkpoint_dir, "dis", iteration=epoch)
        ...
        print("Resume from {} at iteration {}".format(checkpoint_dir, epoch))
        logger.info("Resume from {} at iteration {}".format(checkpoint_dir, epoch))
